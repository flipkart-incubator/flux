...
public class ClientElbResource {
  ...
  public Response createClientElb(@QueryParam("clientId") String clientId,
    ...
    if (clientElbUrl == null || clientId == null) {
      ...
      try {
        ...
        return Response.status(Response.Status.CREATED.getStatusCode()).entity(clientElb.getId())
            .build();
      } catch (MalformedURLException ex) {
        logger.error(
            "Malformed URL exception(no path allowed because Elb Url doesn't contain path) {} {} ",
        ...
        logger.error("Exception occured in ClientElb create {} {} ", ex.getMessage(),
            ex.getStackTrace());
  ...
  public Response findByIdClientElb(@QueryParam("clientId") String clientId) {
    ...
    try {
      ...
      logger.error("findById failed for input: " + clientId + " {} {} ", ex.getMessage(),
          ex.getStackTrace());
      return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
    }
  }
...
/*
Disabling Update and delete because an update on a stateless box doesn't propagate to other stateless boxes
where query didn't arrive. This was relevant when there was no cache for data, with cache in-between
Update should be done as  "Delete from DB, restart daemon svc and insert via ClientElb API"
 */
//  /**
...
//    } catch (Exception ex) {
//      logger.error("Deletion failed for input: " + clientId + " {} {} ", ex.getMessage(),
//          ex.getStackTrace());
//      return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
//    }
//    return Response.status(Response.Status.ACCEPTED).build();
