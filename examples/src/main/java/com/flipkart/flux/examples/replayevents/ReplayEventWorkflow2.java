...
public class ReplayEventWorkflow2 {
	 ...
	 */
	private static final Logger logger = LogManager.getLogger(ReplayEventWorkflow2.class);
	...
	public IntegerEvent t5(@ReplayEvent("RE1") IntegerEvent re1, IntegerEvent e1) {
		logger.info("Executing replayable task t5 with Integer Event : {}", re1);
		if (re1 != null)
	...
	public IntegerEvent t9(@ReplayEvent("RE2") IntegerEvent re2, IntegerEvent e5, IntegerEvent e8) {
		logger.info("Executing replayable task t9 with Integer Event : {}", re2);
		if (re2 != null)
