 ...
 * Things to Note:
 * 1. The workflow essentially pauses at userVerificationService.waitForVerification call. Notice that the passed parameter is null
 ...
 * 5. Notice the implementation of userVerificationService.waitForVerificationService(). It re-sends the same userVerificationStatus.
 * The UserVerificationStatus parameter passed to the method and the one returned by the method are treated as different events by the runtime
 */
public class ManualUserVerificationFlow {
    @Inject
    private UserDataService userDataService;

    @Inject
    private ManualUserVerificationService userVerificationService;
    ...
    public void verifyUser(UserId userId) {
        ...
        notificationService.notifyCustomerSupport(userData);
        ...
        final UserVerificationStatus userVerificationStatus = userVerificationService.waitForVerification(null);
